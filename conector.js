/**  Interface de conexão com API REST JSON
...
/** onOpen cria a interface de conexão ao abrir a planilha
 * - Menu BUSCAR DADOS
 */
function onOpen() {
 SpreadsheetApp
   .getUi()
   .createMenu("BUSCAR DADOS")
   .addItem("BUSCAR", "showAdminSidebar")
   .addItem("ATUALIZAR", "updateSheets")
   .addToUi();
}

/** Cria a Sidebar com o formulário de consulta
 *  com base no arquivo Sidebar.html
 */
function showAdminSidebar() {
 const widget = HtmlService.createHtmlOutputFromFile("Sidebar.html");
  widget.setTitle("FORMULÁRIO");
 SpreadsheetApp.getUi().showSidebar(widget);
}

/** insertData preenche a planilha com os dados retornados pela API
 * @param {nomePlanilha} nome da aba (sheet) da planilha a ser preenchida
 * @param {url} url de consulta a API
 */
function insertData(NomePlanilha, url){
...
/** updateData atualiza os dados de uma aba (sheet) mudando as datas mantendo a mesma janela de tempo.
 * @param (sheetName) nome da aba a ser atualizada
 */
function updateData(sheetName){
     const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(sheetName);
     const cell = sheet.getRange("A1");
     const formula = cell.getFormula();
     const args = formula.match(/=\w+\((.*)\)/i);
     const url = args[1].split("&");
     
     const datainicial = addDays(getInicialDate(url),1)
     const datafinal = new Date().toISOString().slice(0, 10);
     url.splice(-3, 2, "datainicial=" + datainicial, "datafinal=" + datafinal);
     Logger.log(url.join("&"))
     cell.clear();
     cell.setFormula(`=ImportJSON(${url.join("&")})`)
}

/** getInicialDate retorna o parametro datainicial da url de consulta
 */
function getInicialDate(urlArray){
     var inicialDate = urlArray.slice(2,-2)
     
     return inicialDate[0].split("=")[1]
}
/** addDays adiciona um número de dias a uma data
...
/** getSheetNames retora os nomes das abas da planilha
 * @return vetor com os nomes das abas
 */
...
/** updateSheets Atualiza todas as abas da planilha (menos a aba dos Ids)
 * usando a função updateData
 */
function updateSheets(){
     const names = getSheetsNames();
     names.shift();
     Logger.log(names)
...
/**updateIds atualiza a aba de IDs a cada 12h usando uma trigger  */
function updateIds(){
